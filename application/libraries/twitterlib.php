...
class Twitterlib {
	...
	*
	*/
	public function search($cachetime=null, $terms = null)
	{
		if ($terms != null) {
			$this->terms = $terms;
		}
...
//		if( $cachetime == null || ! $content = $this->CI->cache->get('twitter-api-search') )
	...
	*
	*/
	public function searchone($cachetime=null, $terms = null)
	{
		if ($terms != null) {
			$this->terms = $terms;
		}
	...
	*/
	public function process($data=null)
	{
		...
		*/
		...
		$data2['hash'] = md5(json_encode($data2));

		$result2 = $this->CI->db->query('select hash from tweets where hash="'.$data2['hash'].'"');
		if (count($result2->result()) == 0) {
			$result = $this->CI->db->insert('tweets',$data2);
		} else {
			return;
		}
